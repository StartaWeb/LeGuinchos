...
function pdfHeader(doc,titulo){
  doc.setFontSize(40);doc.setTextColor(200,200,200);doc.setFont('helvetica','bold');
  doc.text('Le Guincho',55,145,{angle:45});
  doc.setFontSize(18);doc.setTextColor(0,100,200);doc.text('Le Guincho',14,15);
  doc.setFontSize(10);doc.setTextColor(100,100,100);doc.text('App do Motorista',14,22);
  doc.setFontSize(14);doc.setTextColor(0,0,0);doc.text(titulo,14,32);
...
function gerarPDFServicos(){
  ...
  const rows=servicos.map(s=>[s.motorista,s.seguradora,new Date(s.horario).toLocaleString('pt-BR'),'R$ '+s.valor.toFixed(2),s.desconto+'%',s.pedagio==='sim'?'Sim':'Não','R$ '+s.valorFinal.toFixed(2)]);
  ...
  doc.text(`Valor Total: R$ ${total.toFixed(2)}`,14,fy+7);
