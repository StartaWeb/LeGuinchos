  ...
  try {
    if (_usuarioEditandoId) {
      ...
      try {
        ...
        alert('✅ Usuário pré-cadastrado! Um e-mail de definição de senha foi enviado para ' + email);
      } catch (authErr) {
        alert('⚠️ Usuário salvo no banco, mas o e-mail de senha não pôde ser enviado. Isso acontece se o e-mail ainda não tem uma conta no sistema. Peça para o usuário se cadastrar ou use o Console do Firebase.');
      }
    }
    fecharModal();
  } catch (err) {
...
async function removerUsuario(id, nome) {
  if (!confirm(`⚠️ ATENÇÃO: Deseja realmente remover o usuário "${nome}"? Ele perderá acesso ao sistema imediatamente.`)) return;
  try {
